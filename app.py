...
# --- [신규] 실시간 쇼핑 BEST 100 크롤링 엔진 ---
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_realtime_shopping_v27(category_id):
    ...
    try:
        ...
        # 네이버 쇼핑 BEST 상품명 추출 로직 (구조 변경 대응을 위해 범용 선택자 사용)
        items = soup.select('div[class*="imageProduct_title"] > a') or soup.select('div[class*="category_product_title"] > a')
        return [item.get_text(strip=True) for item in items[:10]]
    except:
        return ["실시간 데이터를 가져오는 중입니다..."]

# --- 기존 데이터 수집 (v27) ---
@st.cache_data(ttl=600, show_spinner=False)
def fetch_keyword_data_v27(target_kw):
    ...
    try:
        ...
        for item in res['keywordList'][:15]:
            ...
            results.append({"v27_kw": kw, "v27_p": p, "v27_m": m, "v27_t": t, "v27_b": b_v, "v27_c": c_v, "v27_d": b_v + c_v, "v27_r": r_v, "v27_idx": round((b_v + c_v) / t, 2) if t > 0 else 0})
        return results
    except: return []
...
if st.session_state.page == "HOME":
    ...
    if submit and input_kw:
        st.session_state.kw_results = fetch_keyword_data_v27(input_kw)
    ...
    if st.session_state.get('kw_results'):
        ...
        try:
            info = next((i for i in res if i['v27_kw'].replace(" ", "") == tgt.replace(" ", "")), res[0])
            ...
            with c1:
                ...
                st.markdown(f"""<div class='quad-box'><div class='quad-title'>🔍 월간 검색량</div><div style='display:flex; justify-content:space-around; text-align:center;'>
                    <div><span class='sub-metric-label'>💻 PC</span><span class='sub-metric-val'>{info['v27_p']:,}</span><span class='sub-pct'>{(info['v27_p']/t*100 if t>0 else 0):.1f}%</span></div>
                    <div><span class='sub-metric-label'>📱 모바일</span><span class='sub-metric-val'>{info['v27_m']:,}</span><span class='sub-pct'>{(info['v27_m']/t*100 if t>0 else 0):.1f}%</span></div>
                    <div><span class='sub-metric-label'>➕ 총합</span><span class='sub-metric-val'>{t:,}</span><span class='sub-pct'>100%</span></div>
                </div></div>""", unsafe_allow_html=True)
            with c2:
                s, col = ("매우 낮음", "#2ecc71") if info['v27_idx'] < 0.5 else ("낮음", "#3498db") if info['v27_idx'] < 1.0 else ("보통", "#f39c12") if info['v27_idx'] < 5.0 else ("높음", "#e67e22") if info['v27_idx'] < 10.0 else ("매우 높음", "#e74c3c")
                st.markdown(f"""<div class='quad-box'><div class='quad-title'>📈 경쟁강도</div><div style='text-align:center;'><span class='metric-val'>{info['v27_idx']}</span><span class='status-badge' style='background-color:{col};'>{s}</span></div></div>""", unsafe_allow_html=True)
            with c3:
                d = info['v27_d']
                st.markdown(f"""<div class='quad-box'><div class='quad-title'>📚 콘텐츠 누적 발행</div><div style='display:flex; justify-content:space-around; text-align:center;'>
                    <div><span class='sub-metric-label'>✍️ 블로그</span><span class='sub-metric-val'>{info['v27_b']:,}</span><span class='sub-pct'>{(info['v27_b']/d*100 if d>0 else 0):.1f}%</span></div>
                    <div><span class='sub-metric-label'>👥 카페</span><span class='sub-metric-val'>{info['v27_c']:,}</span><span class='sub-pct'>{(info['v27_c']/d*100 if d>0 else 0):.1f}%</span></div>
                    <div><span class='sub-metric-label'>➕ 총합</span><span class='sub-metric-val'>{d:,}</span><span class='sub-pct'>100%</span></div>
                </div></div>""", unsafe_allow_html=True)
            with c4:
                st.markdown(f"""<div class='quad-box'><div class='quad-title'>📅 최근 한 달 발행</div><div style='text-align:center;'><span class='metric-val'>{info['v27_r']}</span><span class='sub-metric-label' style='font-size:1.5em; display:inline;'>건</span></div></div>""", unsafe_allow_html=True)
            st.divider()
            df = pd.DataFrame(res)
            m_cols = [("키워드", " "), ("월간 검색량", "PC"), ("월간 검색량", "모바일"), ("월간 검색량", "총합"), ("콘텐츠 누적발행", "블로그"), ("콘텐츠 누적발행", "카페"), ("콘텐츠 누적발행", "총합"), ("최근 한 달\n발행량", " "), ("경쟁강도", " ")]
            df_display = df[["v27_kw", "v27_p", "v27_m", "v27_t", "v27_b", "v27_c", "v27_d", "v27_r", "v27_idx"]]
...
# 2. KEYWORD: 카테고리별 인기 키워드 (기존 정적 데이터)
    ...
    for i in range(0, 8, 4):
        ...
        for j in range(4):
            n, q = items[i+j]
            trends = requests.get(f"https://ac.search.naver.com/nx/ac?q={q}&con=0&ans=2&r_format=json&st=100").json()['items'][0]
            html = "".join([f"<div style='margin-bottom:8px; text-align:left;'><span style='color:#F4B742; font-weight:bold;'>{idx+1}</span> {v[0]}</div>" for idx, v in enumerate(trends[:10])])
...
# 3. TREND: 실시간 쇼핑 트렌드 (진짜 크롤링 반영)
    ...
    for i in range(0, 8, 4):
        ...
        for j in range(4):
            n, cid = items[i+j]
            trends = fetch_realtime_shopping_v27(cid)
            html = "".join([f"<div style='margin-bottom:8px; text-align:left; font-size:0.9em;'><span style='color:#F4B742; font-weight:bold;'>{idx+1}</span> {v}</div>" for idx, v in enumerate(trends)])
            cols[j].markdown(f"<div style='border:1px solid #eee; border-radius:12px; padding:15px; min-height:450px;'><h4>{n}</h4><br>{html}</div>", unsafe_allow_html=True)
